...
- If you are on Linux or Windows NT, your only workspace is `./Packages/vChewing_Typewriter` and its dependencies situated in `./Packages`. If you are on Windows, you can also work with `./Packages/vChewing_MainAssembly4Darwin` and its dependencies situated in `./Packages` folder. Note that the session system (`InputSession`, `SessionProtocol`, `SessionHost`, etc.) now lives in `vChewing_Typewriter` — session-involving development is possible on Linux/Windows, with all OS-dependent actions injected via `SessionHost` closures.
