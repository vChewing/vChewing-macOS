...
- **Legacy static path (Swift 5.10)**: an additional, compilability-only path — the Swift 5.10 toolchain with Xcode 15's `MacOSX13.3.sdk`, targeting `x86_64-apple-macosx10.10` — produces static `.a` archives for the legacy repository's Xcode link step (`libArcLite` and the macOS 10.9-compatible Swift Runtime can only be folded in there). It never produces executables and does not change the authoritative Swift 6.4+ / macOS 12+ setup (Swift 6.2 and 6.3 are deliberately blocked by per-package blocker manifests). Under this rollout each subpackage carries `Package@swift-5.10.swift` plus `Package@swift-6.0` / `6.1` / `6.2` / `6.3` blocker manifests and its own lowercase `makefile` (`make build510`, `make clean510`); the LibVanguard aggregate's set — six manifests — is the reference implementation of that layout. Swift 5.10 builds stay out of CI: each runner would have to install the extra toolchain and the Xcode 15 SDK at no small cost, and the 5.10 side only serves compilability verification, so it is validated locally. Rollout details: `vChewing-DevLogs/Research/Phase217_SOP.md` (where that SOP is silent, `AGENTS.md` governs; conflicts escalate rather than being resolved silently).
