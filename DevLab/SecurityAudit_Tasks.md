...
- [x] Harden Candidate Text Services (High)
  ...
  - 主要檔案：`Packages/vChewing_OSNeutral_LibVanguard/Sources/Shared/CandidateTextService.swift`、`Packages/vChewing_MainAssembly4Darwin/Sources/MainAssembly/CandidateTextService_SelectorImpl.swift`、`Packages/vChewing_MainAssembly4Darwin/Sources/MainAssembly/SessionController/InputSession_Delegates.swift`
  - 建議步驟：
  ...
  - 建議步驟：
    ...
    3. 新增 `CandidateTextService` 的單元測試：
      - 註：目前已新增與強化 `build_darwin_MainAssembly.yml`，使 macOS CI:
        ...
        - 在 macOS Runner 上為關鍵 macOS-only package（`vChewing_OSNeutral_LibVanguard`, `vChewing_MainAssembly4Darwin`, `vChewing_Hotenka`, `vChewing_OSFrameworkImpl`, `vChewing_UpdateSputnik`, `Jad_BookmarkManager`）逐一執行 `swift test`。
...
- `Packages/vChewing_OSNeutral_LibVanguard/Sources/LexiconAssembly/LXFacade_SQLExtension.swift`
