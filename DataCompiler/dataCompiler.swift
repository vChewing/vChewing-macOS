...
private let urlPlistBPMFReverseLookupCNS: String = "./data-bpmf-reverse-lookup-CNS.plist"
...
func commonFileOutput() {
  ...
  var mapReverseLookupCNS: [String: [Data]] = [:]
  // 讀取標點內容
  do {
  ...
  strCNS.ranges(splitBy: "\n").forEach {
    ...
    if neta.count >= 2 {
      ...
      if !neta[0].isEmpty, !neta[1].isEmpty, line.first != "#" {
        ...
        if !theKey.contains("_"), !theKey.contains("-") {
          mapReverseLookupCNS[theValue, default: []].append(cnvPhonabetToASCII(theKey).data(using: .utf8)!)
        }
      }
    }
  }
  ...
  do {
    ...
    try PropertyListSerialization.data(fromPropertyList: mapReverseLookupCNS, format: .binary, options: 0).write(
      to: URL(fileURLWithPath: urlPlistBPMFReverseLookupCNS))
  } catch {
    NSLog(" - \(i18n): Error on writing strings to file: \(error)")
  }
  NSLog(" - \(i18n): 寫入完成。")
