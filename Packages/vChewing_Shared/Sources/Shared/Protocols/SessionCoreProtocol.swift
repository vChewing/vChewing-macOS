...
public protocol SessionCoreProtocol: AnyObject {
  ...
  associatedtype Handler: InputHandlerCoreProtocol where Handler.State == Handler.Session.State, Handler.State == State
...
public protocol InputHandlerCoreProtocol: AnyObject {
  associatedtype State: IMEStateProtocol
  associatedtype Session: SessionCoreProtocol & CtlCandidateDelegate where Session.State == Session.Handler.State,
    Session.State == State
}
