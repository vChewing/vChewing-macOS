...
// 本檔是 `Package.swift`（tools 6.4）的 **Swift 5.10 對位版**：同一份 `Sources/`／`Framework/` 於 Swift 5.10
// toolchain 下以本 manifest 建置，於 Swift 6.4 以上以 `Package.swift` 建置。SwiftPM 之版本擇定規則為
// 「於 `Package*.swift` 全集中，取**檔內宣告**的 tools version 不高於 toolchain 版本者之**最高者**；同版時
...
//
// C 靶 `CapsLockToggler`（`path: "Framework"` ＋ `headerSearchPath("include")`）原樣保留——它是**生產靶**、
// 不是測試靶。環境配對與各項差異的機理見 vChewing-DevLogs/Research/Phase217_SOP.md §三／§四。
