...
extension String {
  public func parseAsInputToken(isCHS: Bool) -> [String] {
    LXAssembly.InputToken.parse(from: self).map { $0.translated(isCHS: isCHS) }.flatMap { $0 }
      .deduplicated
  }
}
...
extension LXAssembly.InputToken {
  static func parse(from rawToken: String) -> [LXAssembly.InputToken] {
    var result: [LXAssembly.InputToken] = []
