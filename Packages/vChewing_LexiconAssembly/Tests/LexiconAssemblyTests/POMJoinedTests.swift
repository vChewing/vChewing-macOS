...
extension POMTestSuite {
  ...
  struct POMJoinedTests {
    ...
    func testPOM_AC01A_SaisoukiNoGaika() throws {
      ...
      // 然後是記憶效力測試：
      let validationLX = TestLX(rawData: HomaTests.strLXSampleData_SaisoukiNoGaika)
      let validationCompositor = Homa.Assembler(
        gramQuerier: validationLX.asGramQuerier()
      )
    ...
    func testPOM_AC02_BusinessEnglishSession() throws {
      let lx = TestLX(rawData: HomaTests.strLXSampleData_BusinessEnglishSession)
      let pom = LXAssembly.LXPerceptor(
        dataURL: URL(fileURLWithPath: "/dev/null")
      )
      let compositor = Homa.Assembler(
      ...
      let validationLX = TestLX(rawData: HomaTests.strLXSampleData_BusinessEnglishSession)
      let validationCompositor = Homa.Assembler(
        gramQuerier: validationLX.asGramQuerier()
      )
    ...
    func testPOM_AC03_DiJiaoSubmission() throws {
      let lx = TestLX(rawData: HomaTests.strLXSampleData_DiJiaoSubmission)
      let pom = LXAssembly.LXPerceptor(dataURL: URL(fileURLWithPath: "/dev/null"))
      let compositor = Homa.Assembler(
        gramQuerier: lx.asGramQuerier()
      )
      ...
      let validationLX = TestLX(rawData: HomaTests.strLXSampleData_DiJiaoSubmission)
      let validationCompositor = Homa.Assembler(
        gramQuerier: validationLX.asGramQuerier()
      )
      for key in readingKeys { try validationCompositor.insertKey(key) }
    ...
    func testPOM_AC04_DuoQiMemorization() throws {
      ...
      ㄑㄧˊ 旗 -5.2
      """
      let unigramLX = TestLX(rawData: unigramOnlyData)
      let pom = LXAssembly.LXPerceptor(dataURL: URL(fileURLWithPath: "/dev/null"))
      let compositor = Homa.Assembler(
        gramQuerier: unigramLX.asGramQuerier()
      )
      ...
      // ====== 記憶效力測試 ======
      let validationLX = TestLX(rawData: unigramOnlyData)
      let validationCompositor = Homa.Assembler(
        gramQuerier: validationLX.asGramQuerier()
      )
      for key in readingKeys { try validationCompositor.insertKey(key) }
      ...
      let multiSegmentLX = TestLX(rawData: LXATestsData.strDataCase4DuoQi)
      let multiSegmentPOM = LXAssembly.LXPerceptor(dataURL: URL(fileURLWithPath: "/dev/null"))
      ...
      let multiSegmentCompositor = Homa.Assembler(
        gramQuerier: multiSegmentLX.asGramQuerier()
      )
