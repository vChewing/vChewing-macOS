...
// MARK: - LXConsolidatorTests

@Suite(.serialized)
struct LXConsolidatorTests {
  // MARK: Internal

  @Test("[LXAssembly] CheckPragma_ValidHeaders")
  func testCheckPragmaValidHeaders() async throws {
    ...
    #expect(Self.withTempFile(contents: "\(header)\ncontent\n") {
      LXAssembly.LXConsolidator.checkPragma(path: $0)
    })
    // CRLF 斷行。
    #expect(Self.withTempFile(contents: "\(header)\r\ncontent\r\n") {
      LXAssembly.LXConsolidator.checkPragma(path: $0)
  ...
  @Test("[LXAssembly] CheckPragma_InvalidHeaders")
  func testCheckPragmaInvalidHeaders() async throws {
    ...
    // 檔案不存在。
    #expect(!LXAssembly.LXConsolidator.checkPragma(
...
// MARK: - LXConsolidatorConsolidateTests

@Suite(.serialized)
struct LXConsolidatorConsolidateTests {
  // MARK: Internal

  @Test("[LXAssembly] ConsolidateText_PragmaIntactSkips")
  func testConsolidateTextPragmaIntactSkips() {
    ...
    for content in [
      ...
      var text = content
      LXAssembly.LXConsolidator.consolidate(text: &text, pragma: true)
      #expect(text == content)
    }
  }

  @Test("[LXAssembly] ConsolidateText_NormalizeWhenHeaderMissing")
  ...
  func testConsolidateTextNewlineClass() {
    ...
    // FF 同時是行錨與可收斂字元。
    var b = "a \u{000C} b"
    LXAssembly.LXConsolidator.consolidate(text: &b, pragma: false)
    #expect(b == "\(header)\na\nb\n")
  ...
  @Test("[LXAssembly] ConsolidateText_Deduplication")
  ...
  @Test("[LXAssembly] ConsolidateText_HeaderLineRemoval")
  ...
  func testConsolidatePathPreservesInvalidUTF8() {
    ...
    defer { try? FileManager.default.removeItem(at: tempURL) }
    #expect(LXAssembly.LXConsolidator.consolidate(path: tempURL.path, pragma: false))
  ...
  private let header = LXAssembly.LXConsolidator.kPragmaHeader
