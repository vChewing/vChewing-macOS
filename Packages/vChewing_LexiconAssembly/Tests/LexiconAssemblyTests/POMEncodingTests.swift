...
extension POMTestSuite {
  ...
  struct POMEncodingTests {
    ...
    func testJSONEncoding() throws {
      ...
      let pom = LXAssembly.LXPerceptor(capacity: 10, dataURL: tempURL)
      ...
      do {
        ...
        let decoded = try JSONDecoder().decode(
          [LXAssembly.LXPerceptor.KeyPerceptionPair].self, from: data2
        )
        #expect(decoded == savableData)
      } catch {
        Issue.record("編碼有資料失敗: \(error)")
      }
    ...
    func testDataIntegrity() throws {
      ...
      #expect(savableData.count == 1)
      #expect(savableData.first?.key == testKey)
      #expect(savableData.first?.perception.overrides.count == 1)
      #expect(savableData.first?.perception.overrides.keys.contains(testCandidate) ?? false)
    }
  }
}
