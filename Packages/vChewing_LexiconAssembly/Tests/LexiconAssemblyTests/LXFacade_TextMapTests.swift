...
// MARK: - LXFacadeTextMapTests

@Suite(.serialized)
struct LXFacadeTextMapTests {
  // MARK: Internal
  ...
  func testTextMapFixtureLoading() throws {
    ...
    #expect(!LXATestsData.textMapTestCoreLXData.isEmpty)
    ...
    #expect(LXAssembly.LXFacade.getFactoryReverseLookupData(with: "和") == expectedReverseLookupResults)
  }
  ...
  func testFactoryReverseLookupAutoGeneratedOrderIsStable() throws {
    ...
    #expect(LXAssembly.LXFacade.connectToTestFactoryDictionary(textMapData: textMap))
    #expect(LXAssembly.LXFacade.getFactoryReverseLookupData(with: "和") == ["ㄏㄜˋ", "ㄏㄨㄛˋ"])
  }
  ...
  func testFactoryTextMapLoadsWithCRLFLineEndings() throws {
    ...
    #expect(LXAssembly.LXFacade.connectToTestFactoryDictionary(textMapData: textMap))
    #expect(LXAssembly.LXFacade.getFactoryReverseLookupData(with: "和") == ["ㄏㄜˋ"])
  }
  ...
  func testFactoryReverseLookupAutoGeneratedUsesSingleSegmentGroupedLinesAndCNSEntries() throws {
    ...
    #expect(LXAssembly.LXFacade.connectToTestFactoryDictionary(textMapData: textMap))
    #expect(LXAssembly.LXFacade.getFactoryReverseLookupData(with: "宜") == ["ㄧˊ"])
    #expect(LXAssembly.LXFacade.getFactoryReverseLookupData(with: "𡜅") == ["ㄌㄩˇ"])
  }
  ...
  func testFactoryDictionaryDoesNotAutoLoadRevLookupFile() async throws {
    ...
    let callbackResult: Bool? = await withCheckedContinuation { continuation in
      LXAssembly.LXFacade.connectFactoryDictionary(textMapPath: textMapPath.path) {
        continuation.resume(returning: $0)
      }
    }

    #expect(callbackResult == true)
    #expect(LXAssembly.LXFacade.getFactoryReverseLookupData(with: "B") == nil)
  }
  ...
  func testFactoryDictionaryRejectsBundledExternalRevLookupFixture() throws {
    ...
    let bundledFixture = LXATestsData.textMapTestCoreLXData
      + "\n#PRAGMA:VANGUARD_REVLOOKUP_TSV\nVERSION\t1\n和\the4\n"

    #expect(!LXAssembly.LXFacade.connectToTestFactoryDictionary(textMapData: bundledFixture))
    #expect(!LXAssembly.LXFacade.connectToTestFactoryDictionary(textMapData: bundledFixture))
  }
  ...
  func testAssemblerFacadeMatchesLegacyUnigramSurface() throws {
    ...
    let assemblerSurface = instance.lxQuerier.grams(for: cakePossibleKeys).map {
      GramSnapshot(keyArray: $0.keyArray, value: $0.current, probability: $0.probability)
    }

    #expect(assemblerSurface == legacySurface)
    #expect(instance.lxQuerier.grams(for: cakePossibleKeys).allSatisfy { $0.previous == nil })
    #expect(instance.lxQuerier.hasGrams(for: strCakeKey) == instance.hasUnigramsFor(keyArray: strCakeKey))
  }
  ...
  func testETenDOSUnigramsDoNotReorderFactoryCoreResults() throws {
    ...
    let supplemental = instance.lxQuerier.supplementalValues(for: "ㄗㄞˋ", strategy: .exactMatch)
  ...
  func testCartesianProductBudgetGateSkipsExpansionBeyondBudget() throws {
    ...
    let big = instance.lxQuerier.grams(for: [
  ...
  func testAbbreviatedWordCandidatesReachesWholeWord() throws {
    ...
    let grams = instance.lxQuerier.abbreviatedWordCandidates(keysChopped: cells)
