...
//    產物的 `LC_BUILD_VERSION`；本側索性不宣告平台限制，讓 SDK 之預設值說話（`Package.swift` 側本就
//    未宣告 `platforms`，此處為求全倉一致仍明列 `nil`）。
// 3. **不含任何測試靶**（Unit Tests 不對 Swift 5.10 開放）：`Package.swift` 的 `KeyKeyUserDBKitTests`
//    整段略去。
