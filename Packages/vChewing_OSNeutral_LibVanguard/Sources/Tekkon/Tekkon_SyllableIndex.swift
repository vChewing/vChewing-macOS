...
extension Tekkon {
  /// 漢語音節之唯讀前綴索引。
  ///
  /// 由 `Tekkon.mapHanyuPinyin` 之注音詞幹（427 條）派生，含其全部非空前綴（442 條）。
  ...
  ///   **辭典之事實**、不是**音節表之事實**（`ㄍ` 只是 `ㄍㄚ` 一族之嚴格前綴，不是獨立音節）。
  ///   故 `isComplete("ㄍ") == false` 而 `isPrefix("ㄍ") == true`。把單符號硬塞進來會使
  ///   「427」這個可稽核數字失去意義，並讓「可否提交」之判斷誤用 `isComplete`。
  ///
  /// - Important: **排列中立**。注音是排列中立之表記，故 427 條詞幹對所有 `MandarinParser`
  ...
  public struct SyllableIndex: Sendable {
    ...
    ///
    /// 以 `allReadings.filter { $0.hasPrefix(prefix) }` 實作——427 條線性掃描，**非熱路徑**
    /// （只在前綴不完整時才需列舉）。
    ///
    ...
    /// 為注音側之來源。實測（Phase 251）：動態排列測試資料之 422 條無調詞幹**全部**皆為
    /// 本表之成員，無一例外。
