# VanguardSwiftExtension

唯音輸入法用到的各種 Swift 功能擴張。**本套件的目錄名、package identity 與 package 名同為
`VanguardSwiftExtension`；target／模組名則為 `SwiftExtension`**（自 4.7.4 以來未變，故全倉
一律 `import SwiftExtension`）；出貨的**產品名**亦為 `VanguardSwiftExtension`。
...
閉包，遂再次析出為獨立套件（現落點為聚合體目錄內的 `Deps/VanguardSwiftExtension/`），並改以
動態產品出貨：
