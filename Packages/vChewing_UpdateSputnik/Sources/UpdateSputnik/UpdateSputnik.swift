...
public final class UpdateSputnik {
  ...
  internal func dataDidSet(data: Data) {
    ...
    guard let plist = plist else {
      asyncOnMain { [weak self] in
        guard let this = self else { return }
        this.showError(message: "Plist downloaded is nil.".localized)
        this.currentTask = nil
      }
      return
    }
    ...
    guard let intRemoteVersion = Int(plist[kCFBundleVersionKey] as? String ?? ""),
      ...
      asyncOnMain { [weak self] in
        guard let this = self else { return }
        this.showError(message: "Plist downloaded cannot be parsed correctly.".localized)
        this.currentTask = nil
      }
      return
    }
    ...
    versionCheck: if intRemoteVersion <= intCurrentVersion {
      ...
      let alert = NSAlert()
      alert.messageText = "Update Check Completed".localized
      alert.informativeText = "You are already using the latest version.".localized
    ...
    var content = String(
      format: "You're currently using vChewing %@ (%@), a new version %@ (%@) is now available. Do you want to visit vChewing's website to download the version?".localized,
    ...
    if crossDistroNotification {
      content.append("\n\n")
      content.append(
        "This update will upgrade vChewing from Aqua Special Edition to Mainstream Release (recommended for your current OS version).".localized
      )
    }
    let alert = NSAlert()
    alert.informativeText = content
    alert.messageText = "New Version Available".localized
    let strVisitWebsite = "Visit Website".localized
