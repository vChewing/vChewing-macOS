...
DictGroup::DictGroup(const std::list<DictPtr> &_dicts) : keyMaxLength(GetKeyMaxLength(_dicts)), dicts(_dicts)
{
}
...
Optional<const DictEntry *> DictGroup::Match(const char *word, size_t len) const
{
...
Optional<const DictEntry *> DictGroup::MatchPrefix(const char *word, size_t len) const
{
    for (const auto &dict : dicts)
    {
...
std::vector<const DictEntry *> DictGroup::MatchAllPrefixes(const char *word, size_t len) const
{
    std::map<size_t, const DictEntry *> matched;
    // Match all prefixes from all dictionaries
    for (const auto &dict : dicts)
    {
        const std::vector<const DictEntry *> &entries = dict->MatchAllPrefixes(word, len);
