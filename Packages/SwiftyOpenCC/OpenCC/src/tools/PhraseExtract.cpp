...
void Extract(const std::vector<std::string> &inputFiles, const std::string &outputFile)
{
    ...
    for (const auto &word : extractor.Words())
    {
        ...
        ofs << word << " " << signals.frequency << " " << logProbablity << " " << signals.cohesion << " " << entropy
            << " " << signals.prefixEntropy << " " << signals.suffixEntropy << std::endl;
    }
    ofs.close();
}
...
int main(int argc, const char *argv[])
{
    try
    {
        TCLAP::CmdLine cmd("Open Chinese Convert (OpenCC) Phrase Extractor", ' ', VERSION);
        CmdLineOutput cmdLineOutput;
        cmd.setOutput(&cmdLineOutput);
        TCLAP::UnlabeledMultiArg<std::string> fileNames("fileName", "Input files", true /* required */, "files");
        cmd.add(fileNames);
        TCLAP::ValueArg<std::string> outputArg("o", "output", "Output file", true /* required */, "" /* default */,
                                               "file" /* type */, cmd);
        cmd.parse(argc, argv);
