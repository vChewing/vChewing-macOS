...
{
  public:
    StrSingleValueDictEntry(const std::string &_key, const std::string &_value) : key(_key), value(_value)
    {
    }
...
class OPENCC_EXPORT DictEntryFactory
{
  public:
    ...
    static DictEntry *New(const std::string &key, const std::vector<std::string> &values)
    {
        if (values.size() == 0)
        {
            return New(key);
