...
inline size_t FNVHash(const char *text, const size_t byteLength, const size_t FNV_prime, const size_t FNV_offset_basis)
{
...
template <> inline size_t FNVHash<4>(const char *text, const size_t byteLength)
{
    return FNVHash(text, byteLength, 16777619UL, 2166136261UL);
}

#if SIZE_MAX == 0xffffffffffffffff
template <> inline size_t FNVHash<8>(const char *text, const size_t byteLength)
{
...
template <typename LENGTH_TYPE> class UTF8StringSliceBase
{
  public:
    ...
    UTF8StringSliceBase(const char *_str, const LengthType _utf8Length) : str(_str), utf8Length(_utf8Length)
    {
        CalculateByteLength();
    }
    ...
    LengthType CommonPrefixLength(const UTF8StringSliceBase &that) const
    {
        ...
        else
        {
            ...
            for (size_t length = 0; length < utf8Length && length < that.utf8Length; length++)
            {
    ...
    {
        return static_cast<LengthType>(ToString().find(pattern.str, 0, pattern.byteLength));
    }
    ...
    bool operator==(const UTF8StringSliceBase &that) const
    {
        return (str == that.str && utf8Length == that.utf8Length) || Compare(that) == 0;
    }
    ...
    class Hasher
    {
      public:
        ...
        {
            return internal::FNVHash<sizeof(size_t)>(text.CString(), text.ByteLength());
        }
    };
...
std::ostream &operator<<(::std::ostream &os, const UTF8StringSliceBase<LENGTH_TYPE> &str)
{
