...
{
  protected:
    UTF8StringSliceTest() : text("天行健，君子以自強不息。地勢坤，君子以厚德載物。"), empty(""){};
...
{
    text.MoveRight();
    EXPECT_EQ(UTF8StringSlice("行健，君子以自強不息。地勢坤，君子以厚德載物。"), text);
...
{
    text.MoveLeft();
    EXPECT_EQ(UTF8StringSlice("天行健，君子以自強不息。地勢坤，君子以厚德載物"), text);
...
TEST_F(UTF8StringSliceTest, FindBytePosition)
{
    ...
    EXPECT_EQ(static_cast<UTF8StringSlice::LengthType>(-1), text.FindBytePosition("a"));
    EXPECT_EQ(static_cast<UTF8StringSlice::LengthType>(-1), text.FindBytePosition("\n"));
