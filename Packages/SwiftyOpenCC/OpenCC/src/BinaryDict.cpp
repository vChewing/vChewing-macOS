...
void BinaryDict::SerializeToFile(FILE *fp) const
{
    ...
    ConstructBuffer(keyBuf, keyOffsets, keyTotalLength, valueBuf, valueOffsets, valueTotalLength);
    // Number of items
...
BinaryDictPtr BinaryDict::NewFromFile(FILE *fp)
{
    ...
    unitsRead = fread(const_cast<char *>(dict->keyBuffer.c_str()), sizeof(char), keyTotalLength, fp);
    ...
    unitsRead = fread(const_cast<char *>(dict->valueBuffer.c_str()), sizeof(char), valueTotalLength, fp);
...
void BinaryDict::ConstructBuffer(std::string &keyBuf, std::vector<size_t> &keyOffset, size_t &keyTotalLength,
                                 std::string &valueBuf, std::vector<size_t> &valueOffset,
