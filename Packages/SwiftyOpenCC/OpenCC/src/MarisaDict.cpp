...
Optional<const DictEntry *> MarisaDict::Match(const char *word, size_t len) const
{
    if (len > maxLength)
    {
...
std::vector<const DictEntry *> MarisaDict::MatchAllPrefixes(const char *word, size_t len) const
{
    const marisa::Trie &trie = *internal->marisa;
    marisa::Agent agent;
    agent.set_query(word, (std::min)(maxLength, len));
...
MarisaDictPtr MarisaDict::NewFromFile(FILE *fp)
{
    ...
    std::shared_ptr<SerializedValues> serialized_values = SerializedValues::NewFromFile(fp);
    ...
    while (dict->internal->marisa->predictive_search(agent))
    {
        ...
        std::unique_ptr<DictEntry> entry(DictEntryFactory::New(key, values_lexicon->At(id)->Values()));
...
void MarisaDict::SerializeToFile(FILE *fp) const
{
    ...
    std::unique_ptr<SerializedValues> serialized_values(new SerializedValues(lexicon));
