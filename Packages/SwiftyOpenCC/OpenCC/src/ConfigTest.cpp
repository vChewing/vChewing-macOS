...
{
  protected:
    ConfigTest() : input(utf8("燕燕于飞差池其羽之子于归远送于野")), expected(utf8("燕燕于飛差池其羽之子于歸遠送於野"))
    {
    }

    virtual void SetUp()
    {
        converter = config.NewFromFile(CONFIG_TEST_PATH);
    }
...
TEST_F(ConfigTest, NewFromStringWitoutTrailingSlash)
{
    ...
    const ConverterPtr _ = config.NewFromString(content, pathWithoutTrailingSlash);
}
