...
Optional<const DictEntry *> DartsDict::Match(const char *word, size_t len) const
{
    if (len > maxLength)
    {
    ...
    if (result.value != -1)
    {
        return Optional<const DictEntry *>(lexicon->At(static_cast<size_t>(result.value)));
    }
    else
    {
        return Optional<const DictEntry *>::Null();
...
Optional<const DictEntry *> DartsDict::MatchPrefix(const char *word, size_t len) const
{
    ...
    size_t numMatched = dict.commonPrefixSearch(word, results, DEFAULT_NUM_ENTRIES, (std::min)(maxLength, len));
    if (numMatched == 0)
    {
        return Optional<const DictEntry *>::Null();
    }
    ...
    {
        Darts::DoubleArray::value_type *rematchedResults = new Darts::DoubleArray::value_type[numMatched];
        numMatched = dict.commonPrefixSearch(word, rematchedResults, numMatched, (std::min)(maxLength, len));
        maxMatchedResult = rematchedResults[numMatched - 1];
        delete[] rematchedResults;
    }
    if (maxMatchedResult >= 0)
    {
        return Optional<const DictEntry *>(lexicon->At(static_cast<size_t>(maxMatchedResult)));
    }
    else
    {
        return Optional<const DictEntry *>::Null();
