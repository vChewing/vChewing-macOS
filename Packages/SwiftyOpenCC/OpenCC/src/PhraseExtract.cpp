...
bool ContainsPunctuation(const PhraseExtract::UTF8StringSlice8Bit &word)
{
    ...
    {
        if (word.FindBytePosition(punctuation) != static_cast<PhraseExtract::UTF8StringSlice8Bit::LengthType>(-1))
        {
            return true;
        }
    }
...
class PhraseExtract::DictType
{
  ...
  private:
    void BuildKeys()
    {
        ...
        ClearDict();
        std::sort(items.begin(), items.end(), [](const ItemType &a, const ItemType &b) { return a.first < b.first; });
    }

    void ClearDict()
    {
        std::unordered_map<UTF8StringSlice8Bit, PhraseExtract::Signals, UTF8StringSlice8Bit::Hasher>().swap(dict);
    }
    ...
    std::unordered_map<UTF8StringSlice8Bit, PhraseExtract::Signals, UTF8StringSlice8Bit::Hasher> dict;
...
bool PhraseExtract::DefaultPreCalculationFilter(const PhraseExtract &, const PhraseExtract::UTF8StringSlice8Bit &)
{
    return false;
}

bool PhraseExtract::DefaultPostCalculationFilter(const PhraseExtract &phraseExtract,
                                                 const PhraseExtract::UTF8StringSlice8Bit &word)
{
    ...
    const double entropyScore = sqrt((signals.prefixEntropy) * (signals.suffixEntropy + 1)) - logProbability * 0.85;
    bool accept = cohesionScore > 9 && entropyScore > 11 && signals.prefixEntropy > 0.5 && signals.suffixEntropy > 0 &&
                  signals.prefixEntropy + signals.suffixEntropy > 3;
    return !accept;
}

PhraseExtract::PhraseExtract()
    : wordMinLength(2), wordMaxLength(2), prefixSetLength(1), suffixSetLength(1),
      preCalculationFilter(DefaultPreCalculationFilter), postCalculationFilter(DefaultPostCalculationFilter),
      utf8FullText(""), signals(new DictType)
{
    Reset();
}
...
{
    suffixes.reserve(utf8FullText.UTF8Length() / 2 * (wordMaxLength + suffixSetLength));
    for (UTF8StringSlice text = utf8FullText; text.UTF8Length() > 0; text.MoveRight())
    {
        const LengthType suffixLength =
            (std::min)(static_cast<LengthType>(wordMaxLength + suffixSetLength), text.UTF8Length());
...
{
    prefixes.reserve(utf8FullText.UTF8Length() / 2 * (wordMaxLength + prefixSetLength));
    for (UTF8StringSlice text = utf8FullText; text.UTF8Length() > 0; text.MoveLeft())
    {
        const LengthType prefixLength =
            (std::min)(static_cast<LengthType>(wordMaxLength + prefixSetLength), text.UTF8Length());
    ...
    std::sort(prefixes.begin(), prefixes.end(),
              [](const UTF8StringSlice8Bit &a, const UTF8StringSlice8Bit &b) { return a.ReverseCompare(b) < 0; });
...
typedef std::unordered_map<PhraseExtract::UTF8StringSlice8Bit, size_t, PhraseExtract::UTF8StringSlice8Bit::Hasher>
    AdjacentSetType;

template <bool SUFFIX>
void CalculatePrefixSuffixEntropy(
    const std::vector<PhraseExtract::UTF8StringSlice8Bit> &presuffixes, const PhraseExtract::LengthType setLength,
    const PhraseExtract::LengthType wordMinLength, const PhraseExtract::LengthType wordMaxLength,
    const std::function<void(const PhraseExtract::UTF8StringSlice8Bit &word, AdjacentSetType &adjacentSet)>
        &updateEntropy)
{
    AdjacentSetType adjacentSet;
    auto setLength8Bit = static_cast<PhraseExtract::UTF8StringSlice8Bit::LengthType>(setLength);
    for (PhraseExtract::LengthType length = wordMinLength; length <= wordMaxLength; length++)
    {
        adjacentSet.clear();
        ...
        for (const auto &presuffix : presuffixes)
        {
            ...
            const auto &wordCandidate = SUFFIX ? presuffix.Left(length8Bit) : presuffix.Right(length8Bit);
            ...
            if (length + setLength <= presuffix.UTF8Length())
            {
                ...
                {
                    const auto &wordPrefix =
                        presuffix.SubString(presuffix.UTF8Length() - length8Bit - setLength8Bit, setLength8Bit);
...
void PhraseExtract::CalculateSuffixEntropy()
{
    ...
    CalculatePrefixSuffixEntropy<true>(
        ...
        [this](const PhraseExtract::UTF8StringSlice8Bit &word, AdjacentSetType &adjacentSet) {
...
const PhraseExtract::Signals &PhraseExtract::Signal(const UTF8StringSlice8Bit &wordCandidate) const
{
...
double PhraseExtract::PMI(const UTF8StringSlice8Bit &wordCandidate, const UTF8StringSlice8Bit &part1,
                          const UTF8StringSlice8Bit &part2) const
{
    // PMI(x, y) = log(P(x, y) / (P(x) * P(y)))
    //           = log(P(x, y)) - log(P(x)) - log(P(y))
    return LogProbability(wordCandidate) - LogProbability(part1) - LogProbability(part2);
}

double PhraseExtract::CalculateCohesion(const UTF8StringSlice8Bit &wordCandidate) const
{
    // TODO Try average value
    double minPMI = INFINITY;
    for (UTF8StringSlice8Bit::LengthType leftLength = 1; leftLength <= wordCandidate.UTF8Length() - 1; leftLength++)
    {
        const auto &leftPart = wordCandidate.Left(leftLength);
        const auto &rightPart = wordCandidate.Right(wordCandidate.UTF8Length() - leftLength);
...
double PhraseExtract::CalculateEntropy(
    const std::unordered_map<UTF8StringSlice8Bit, size_t, UTF8StringSlice8Bit::Hasher> &choices) const
{
