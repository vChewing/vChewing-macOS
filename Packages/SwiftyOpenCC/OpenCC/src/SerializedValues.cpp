...
{
    std::shared_ptr<SerializedValues> dict(new SerializedValues(LexiconPtr(new Lexicon)));
    ...
    size_t unitsRead = fread(const_cast<char *>(valueBuffer.c_str()), sizeof(char), valueTotalLength, fp);
...
void SerializedValues::ConstructBuffer(std::string *valueBuffer, std::vector<uint16_t> *valueBytes,
