...
{
  protected:
    PhraseExtractTest() : siShi(utf8("四是四十是十十四是十四四十是四十")), punctuation(utf8("一.二.三"))
    {
    }
...
TEST_F(PhraseExtractTest, ExtractSuffixes)
{
    ...
    EXPECT_EQ(std::vector<UTF8StringSlice8Bit>({"十", "十十四是", "十四四十", "十四是十", "十是十十", "十是四十",
...
TEST_F(PhraseExtractTest, ExtractPrefixes)
{
    ...
    EXPECT_EQ(std::vector<UTF8StringSlice8Bit>({"十是十十", "十四四十", "十是四十", "四是四十", "四十是十", "十四是十",
...
TEST_F(PhraseExtractTest, ExtractWordCandidates)
{
    ...
    EXPECT_EQ(
        std::vector<UTF8StringSlice8Bit>({"十",     "四",   "是",     "四十",   "十四",   "十是",   "四十是", "四是",
                                          "是十",   "是四", "是四十", "十十",   "十十四", "十四四", "十四是", "十是十",
...
TEST_F(PhraseExtractTest, SelectWords)
{
    ...
    phraseExtract.SetWordMaxLength(3);
    phraseExtract.SetFullText(siShi);
    phraseExtract.SetPostCalculationFilter([](const PhraseExtract &phraseExtract, const UTF8StringSlice8Bit &word) {
        return phraseExtract.Frequency(word) == 1;
    });
    phraseExtract.SelectWords();
    EXPECT_EQ(std::vector<UTF8StringSlice8Bit>(
                  {"十", "四", "是", "四十", "十四", "十是", "四十是", "四是", "是十", "是四", "是四十"}),
              phraseExtract.Words());
}
...
TEST_F(PhraseExtractTest, Punctuation)
{
    ...
    phraseExtract.ExtractPrefixes();
    EXPECT_EQ(std::vector<UTF8StringSlice8Bit>({"一.", ".二.", "一", "二.三", "一.二"}), Prefixes());
}
