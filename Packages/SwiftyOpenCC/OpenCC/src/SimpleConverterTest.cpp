...
class SimpleConverterTest : public ConfigTestBase
{
  protected:
    ...
    void TestConverter(const std::string &config) const
    {
        ...
        const std::string &converted = converter.Convert(utf8("燕燕于飞差池其羽之子于归远送于野"));
        EXPECT_EQ(utf8("燕燕于飛差池其羽之子于歸遠送於野"), converted);
    }
};

TEST_F(SimpleConverterTest, Convert)
{
    TestConverter(CONFIG_TEST_PATH);
}

TEST_F(SimpleConverterTest, Multithreading)
{
    const auto &routine = [this](const std::string &config) { TestConverter(config); };
...
TEST_F(SimpleConverterTest, CInterface)
{
    ...
    }
    {
        char output[1024];
        opencc_t od = opencc_open(CONFIG_TEST_PATH.c_str());
        size_t length = opencc_convert_utf8_to_buffer(od, text.c_str(), (size_t)-1, output);
