...
{
  protected:
    DartsDictTest() : dartsDict(DartsDict::NewFromDict(*textDict.get())), fileName("dict.ocd"){};

    const DartsDictPtr dartsDict;
    const std::string fileName;
};

TEST_F(DartsDictTest, DictTest)
{
    TestDict(dartsDict);
}
...
{
    const DartsDictPtr &deserialized = SerializableDict::NewFromFile<DartsDict>(fileName);
