...
Keyset::Keyset()
    : base_blocks_(), base_blocks_size_(0), base_blocks_capacity_(0), extra_blocks_(), extra_blocks_size_(0),
      extra_blocks_capacity_(0), key_blocks_(), key_blocks_size_(0), key_blocks_capacity_(0), ptr_(NULL), avail_(0),
      size_(0), total_length_(0)
{
}
...
void Keyset::append_base_block()
{
    ...
    {
        const std::size_t new_capacity = (base_blocks_size_ != 0) ? (base_blocks_size_ * 2) : 1;
        scoped_array<scoped_array<char>> new_blocks(new (std::nothrow) scoped_array<char>[new_capacity]);
...
void Keyset::append_extra_block(std::size_t size)
{
    ...
    {
        const std::size_t new_capacity = (extra_blocks_size_ != 0) ? (extra_blocks_size_ * 2) : 1;
        scoped_array<scoped_array<char>> new_blocks(new (std::nothrow) scoped_array<char>[new_capacity]);
        MARISA_THROW_IF(new_blocks.get() == NULL, MARISA_MEMORY_ERROR);
...
void Keyset::append_key_block()
{
    ...
    {
        const std::size_t new_capacity = (key_blocks_size_ != 0) ? (key_blocks_size_ * 2) : 1;
        scoped_array<scoped_array<Key>> new_blocks(new (std::nothrow) scoped_array<Key>[new_capacity]);
        MARISA_THROW_IF(new_blocks.get() == NULL, MARISA_MEMORY_ERROR);
