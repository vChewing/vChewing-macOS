...
LoudsTrie::LoudsTrie()
    : louds_(), terminal_flags_(), link_flags_(), bases_(), extras_(), tail_(), next_trie_(), cache_(), cache_mask_(0),
      num_l1_nodes_(0), config_(), mapper_()
{
}
...
void LoudsTrie::reverse_lookup(Agent &agent) const
{
    ...
    for (;;)
    {
        if (link_flags_[state.node_id()])
        {
            ...
            std::reverse(state.key_buf().begin() + prev_key_pos, state.key_buf().end());
        }
        else
        {
...
{
    return louds_.total_size() + terminal_flags_.total_size() + link_flags_.total_size() + bases_.total_size() +
           extras_.total_size() + tail_.total_size() + ((next_trie_.get() != NULL) ? next_trie_->total_size() : 0) +
           cache_.total_size();
}

std::size_t LoudsTrie::io_size() const
{
    return Header().io_size() + louds_.io_size() + terminal_flags_.io_size() + link_flags_.io_size() +
           bases_.io_size() + extras_.io_size() + tail_.io_size() +
           ((next_trie_.get() != NULL) ? (next_trie_->io_size() - Header().io_size()) : 0) + cache_.io_size() +
           (sizeof(UInt32) * 2);
}
...
template <typename T>
void LoudsTrie::build_trie(Vector<T> &keys, Vector<UInt32> *terminals, const Config &config, std::size_t trie_id)
{
    ...
    {
        config_.parse(static_cast<int>((next_trie_->num_tries() + 1)) | next_trie_->tail_mode() |
                      next_trie_->node_order());
    }
    else
    {
        config_.parse(1 | tail_.mode() | config.node_order() | config.cache_level());
    }
...
template <typename T>
void LoudsTrie::build_current_trie(Vector<T> &keys, Vector<UInt32> *terminals, const Config &config,
                                   std::size_t trie_id)
try
{
    ...
    while (!queue.empty())
    {
        ...
        for (std::size_t i = range.begin() + 1; i < range.end(); ++i)
        {
            ...
            {
                w_ranges.push_back(make_weighted_range(range.begin(), i, range.key_pos(), (float)weight));
                range.set_begin(i);
                weight = 0.0;
            }
            weight += keys[i].weight();
        }
        w_ranges.push_back(make_weighted_range(range.begin(), range.end(), range.key_pos(), (float)weight));
        if (config.node_order() == MARISA_WEIGHT_ORDER)
        {
            std::stable_sort(w_ranges.begin(), w_ranges.end(), std::greater<WeightedRange>());
        }
        ...
        for (std::size_t i = 0; i < w_ranges.size(); ++i)
        {
            ...
            cache<T>(node_id, bases_.size(), w_range.weight(), keys[w_range.begin()][w_range.key_pos()]);

            if (key_pos == w_range.key_pos() + 1)
            {
                bases_.push_back(static_cast<unsigned char>(keys[w_range.begin()][w_range.key_pos()]));
                link_flags_.push_back(false);
            }
            else
            {
                ...
                T next_key;
                next_key.set_str(keys[w_range.begin()].ptr(), keys[w_range.begin()].length());
...
template <>
void LoudsTrie::build_next_trie(Vector<Key> &keys, Vector<UInt32> *terminals, const Config &config, std::size_t trie_id)
{
...
template <>
void LoudsTrie::build_next_trie(Vector<ReverseKey> &keys, Vector<UInt32> *terminals, const Config &config,
                                std::size_t trie_id)
{
    if (trie_id == config.num_tries())
    {
...
template <typename T> void LoudsTrie::build_terminals(const Vector<T> &keys, Vector<UInt32> *terminals) const
{
    Vector<UInt32> temp;
...
template <> void LoudsTrie::cache<Key>(std::size_t parent, std::size_t child, float weight, char label)
{
...
void LoudsTrie::reserve_cache(const Config &config, std::size_t trie_id, std::size_t num_keys)
{
...
template <> void LoudsTrie::cache<ReverseKey>(std::size_t parent, std::size_t child, float weight, char)
{
    MARISA_DEBUG_IF(parent >= child, MARISA_RANGE_ERROR);
...
void LoudsTrie::fill_cache()
{
    for (std::size_t i = 0; i < cache_.size(); ++i)
    {
        ...
        if (node_id != 0)
        {
            ...
            cache_[i].set_extra(!link_flags_[node_id] ? MARISA_INVALID_EXTRA : extras_[link_flags_.rank1(node_id)]);
        }
        else
        {
...
{
    MARISA_DEBUG_IF(agent.state().query_pos() >= agent.query().length(), MARISA_BOUND_ERROR);

    State &state = agent.state();
    const std::size_t cache_id = get_cache_id(state.node_id(), agent.query()[state.query_pos()]);
    ...
    do
    {
        ...
        else if (bases_[state.node_id()] == (UInt8)agent.query()[state.query_pos()])
        {
            state.set_query_pos(state.query_pos() + 1);
            return true;
        }
...
std::size_t LoudsTrie::get_link(std::size_t node_id, std::size_t link_id) const
{
    return bases_[node_id] | (extras_[link_id] * 256);
}

std::size_t LoudsTrie::update_link_id(std::size_t link_id, std::size_t node_id) const
{
    return (link_id == MARISA_INVALID_LINK_ID) ? link_flags_.rank1(node_id) : (link_id + 1);
}
