...
class BitVector
{
  public:
    ...
    BitVector() : units_(), size_(0), num_1s_(0), ranks_(), select0s_(), select1s_()
    {
    }
    ...
    bool operator[](std::size_t i) const
    {
        ...
        return (units_[i / MARISA_WORD_SIZE] & ((Unit)1 << (i % MARISA_WORD_SIZE))) != 0;
    }
    ...
    {
        return units_.total_size() + ranks_.total_size() + select0s_.total_size() + select1s_.total_size();
    }
    std::size_t io_size() const
    {
        return units_.io_size() + (sizeof(UInt32) * 2) + ranks_.io_size() + select0s_.io_size() + select1s_.io_size();
    }
  ...
  private:
    ...
    void build_index(const BitVector &bv, bool enables_select0, bool enables_select1);
