...
template <class Lambda> class LambdaBenchmark : public Benchmark
{
  ...
  private:
    template <class Lam> friend Benchmark * ::benchmark::RegisterBenchmark(const char *, Lam &&);
...
class BenchmarkReporter
{
  public:
    ...
    struct Run
    {
        ...
        Run()
            : run_type(RT_Iteration), error_occurred(false), iterations(1), threads(1), time_unit(kNanosecond),
              real_accumulated_time(0), cpu_accumulated_time(0), max_heapbytes_used(0), complexity(oNone),
              complexity_lambda(), complexity_n(0), report_big_o(false), report_rms(false), counters(),
              has_memory_result(false), allocs_per_iter(0.0), max_bytes_used(0)
        {
        }
