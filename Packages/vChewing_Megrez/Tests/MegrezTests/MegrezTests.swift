...
@Suite("MegrezTestsBasic")
final class MegrezTestsBasic {
  @Test
  ...
  func test02_Compositor_BasicSegmentNodeGramInsertion() throws {
    ...
    #expect(compositor.segments.count == 1)
    #expect(compositor.segments[0].maxLength == 1)
    #expect(compositor.segments[0][1]?.keyArray == ["s"])
    compositor.dropKey(direction: .rear)
    #expect(compositor.cursor == 0)
    #expect(compositor.length == 0)
    #expect(compositor.segments.isEmpty)
  }
  ...
  func test03_Compositor_DefendingInvalidOps() throws {
    ...
    #expect(!compositor.insertKey("guo3"))
    ...
    #expect(compositor.insertKey("ping2"))
    #expect(compositor.dropKey(direction: .rear))
    #expect(compositor.length == 0)
    #expect(compositor.insertKey("ping2"))
    compositor.cursor = 0
  ...
  func test04_Compositor_SegmentsAcrossPositions() throws {
    ...
    #expect((compositor.cursor, compositor.length) == (3, 3))
    #expect((compositor.segments.count) == 3)
    #expect(compositor.segments[0].maxLength == 3)
    #expect(compositor.segments[0][1]?.keyArray == ["h"])
    #expect(compositor.segments[0][2]?.keyArray == ["h", "o"])
    #expect(compositor.segments[0][3]?.keyArray == ["h", "o", "g"])
    #expect(compositor.segments[1].maxLength == 2)
    #expect(compositor.segments[1][1]?.keyArray == ["o"])
    #expect(compositor.segments[1][2]?.keyArray == ["o", "g"])
    #expect(compositor.segments[2].maxLength == 1)
    #expect(compositor.segments[2][1]?.keyArray == ["g"])
  }
  ...
  func test05_Compositor_KeyAndSegmentDeletionInAllDirections() throws {
    ...
    do {
      ...
      #expect(!compositor.dropKey(direction: .front))
      #expect(compositor.dropKey(direction: .rear))
      #expect((compositor.cursor, compositor.length) == (2, 2))
      #expect((compositor.segments.count) == 2)
      #expect(compositor.segments[0].maxLength == 2)
      #expect(compositor.segments[0][1]?.keyArray == ["h"])
      #expect(compositor.segments[0][2]?.keyArray == ["h", "o"])
      #expect(compositor.segments[1].maxLength == 1)
      #expect(compositor.segments[1][1]?.keyArray == ["o"])
    }
    ...
    do {
      ...
      #expect(!compositor.dropKey(direction: .rear))
      #expect(compositor.dropKey(direction: .front))
      #expect((compositor.cursor, compositor.length) == (0, 2))
      #expect((compositor.segments.count) == 2)
      #expect(compositor.segments[0].maxLength == 2)
      #expect(compositor.segments[0][1]?.keyArray == ["o"])
      #expect(compositor.segments[0][2]?.keyArray == ["o", "g"])
      #expect(compositor.segments[1].maxLength == 1)
      #expect(compositor.segments[1][1]?.keyArray == ["g"])
    }
    ...
    do {
      ...
      compositor.cursor = 2
      #expect(compositor.dropKey(direction: .rear))
      #expect((compositor.cursor, compositor.length) == (1, 2))
      #expect((compositor.segments.count) == 2)
      #expect(compositor.segments[0].maxLength == 2)
      #expect(compositor.segments[0][1]?.keyArray == ["h"])
      #expect(compositor.segments[0][2]?.keyArray == ["h", "g"])
      #expect(compositor.segments[1].maxLength == 1)
      #expect(compositor.segments[1][1]?.keyArray == ["g"])
    }
    ...
    do {
      ...
      #expect(compositor.segments[1].maxLength == 1)
      #expect(compositor.segments[1][1]?.keyArray == ["g"])
      #expect(snapshot == compositor.config)
    }
  }
  ...
  func test06_Compositor_SegmentInsertion() throws {
    ...
    #expect((compositor.cursor, compositor.length) == (2, 4))
    #expect(compositor.segments.count == 4)
    #expect(compositor.segments[0].maxLength == 4)
    #expect(compositor.segments[0][1]?.keyArray == ["是"])
    #expect(compositor.segments[0][2]?.keyArray == ["是", "大"])
    #expect(compositor.segments[0][3]?.keyArray == ["是", "大", "學"])
    #expect(compositor.segments[0][4]?.keyArray == ["是", "大", "學", "生"])
    #expect(compositor.segments[1].maxLength == 3)
    #expect(compositor.segments[1][1]?.keyArray == ["大"])
    #expect(compositor.segments[1][2]?.keyArray == ["大", "學"])
    #expect(compositor.segments[1][3]?.keyArray == ["大", "學", "生"])
    #expect(compositor.segments[2].maxLength == 2)
    #expect(compositor.segments[2][1]?.keyArray == ["學"])
    #expect(compositor.segments[2][2]?.keyArray == ["學", "生"])
    #expect(compositor.segments[3].maxLength == 1)
    #expect(compositor.segments[3][1]?.keyArray == ["生"])
  }
  ...
  func test07_Compositor_LongGridDeletionAndInsertion() throws {
    ...
    do {
      compositor.cursor = 8
      #expect(compositor.dropKey(direction: .rear))
      #expect((compositor.cursor, compositor.length) == (7, 13))
      #expect(compositor.segments.count == 13)
      #expect(compositor.segments[0][5]?.keyArray.joined() == "無可奈何花")
      #expect(compositor.segments[1][5]?.keyArray.joined() == "可奈何花作")
      #expect(compositor.segments[2][5]?.keyArray.joined() == "奈何花作香")
      #expect(compositor.segments[3][5]?.keyArray.joined() == "何花作香蝶")
      #expect(compositor.segments[4][5]?.keyArray.joined() == "花作香蝶能")
      #expect(compositor.segments[5][5]?.keyArray.joined() == "作香蝶能留")
      #expect(compositor.segments[6][5]?.keyArray.joined() == "香蝶能留一")
      #expect(compositor.segments[7][5]?.keyArray.joined() == "蝶能留一縷")
      #expect(compositor.segments[8][5]?.keyArray.joined() == "能留一縷芳")
    }
    do {
      #expect(compositor.insertKey("幽"))
      #expect((compositor.cursor, compositor.length) == (8, 14))
      #expect(compositor.segments.count == 14)
      #expect(compositor.segments[0][6]?.keyArray.joined() == "無可奈何花作")
      #expect(compositor.segments[1][6]?.keyArray.joined() == "可奈何花作香")
      #expect(compositor.segments[2][6]?.keyArray.joined() == "奈何花作香幽")
      #expect(compositor.segments[3][6]?.keyArray.joined() == "何花作香幽蝶")
      #expect(compositor.segments[4][6]?.keyArray.joined() == "花作香幽蝶能")
      #expect(compositor.segments[5][6]?.keyArray.joined() == "作香幽蝶能留")
      #expect(compositor.segments[6][6]?.keyArray.joined() == "香幽蝶能留一")
      #expect(compositor.segments[7][6]?.keyArray.joined() == "幽蝶能留一縷")
      #expect(compositor.segments[8][6]?.keyArray.joined() == "蝶能留一縷芳")
    }
  }
}
...
@Suite("MegrezTestsAdvanced")
final class MegrezTestsAdvanced {
  ...
  func test08_Compositor_WordSegmentation() throws {
    ...
    #expect(hardCopy.config == compositor.config)
  }
  ...
  func test13_Compositor_AssembleAndOverrideWithUnigramAndCursorJump() throws {
    ...
    #expect(compositor.length == 12)
    #expect(compositor.length == compositor.cursor)
    // 初始組句結果。
    ...
    #expect(compositor.jumpCursorBySegment(to: .rear))
    #expect(compositor.cursor == 9)
    #expect(compositor.jumpCursorBySegment(to: .rear))
    #expect(compositor.cursor == 8)
    #expect(compositor.jumpCursorBySegment(to: .rear))
    #expect(compositor.cursor == 7)
    #expect(compositor.jumpCursorBySegment(to: .rear))
    ...
    #expect(compositor.cursor == 0)
    #expect(!compositor.jumpCursorBySegment(to: .rear))
    #expect(compositor.cursor == 0) // 接下來準備向前
    #expect(compositor.jumpCursorBySegment(to: .front))
    #expect(compositor.cursor == 2)
    #expect(compositor.jumpCursorBySegment(to: .front))
    #expect(compositor.cursor == 5)
    #expect(compositor.jumpCursorBySegment(to: .front))
  ...
  func test14_Compositor_AssembleAndOverride_AnotherTest() throws {
    ...
    旅 -> 芳;\n呂方;\n呂方 -> EOS;\n芳;\n芳 -> EOS;\nEOS;\n}\n
    """
    #expect(compositor.dumpDOT == expectedDOT)
  }
  ...
  func test17_Compositor_CandidateDisambiguation() throws {
    ...
    var result = compositor.assemble()
    #expect(result.values == ["大樹", "新的", "蜜蜂"])
    let pos = 2

    #expect(compositor.overrideCandidate(.init(keyArray: ["xin1"], value: "🆕"), at: pos))
    result = compositor.assemble()
    #expect(result.values == ["大樹", "🆕", "的", "蜜蜂"])

    #expect(
      compositor.overrideCandidate(.init(keyArray: ["xin1", "de5"], value: "🆕"), at: pos)
    )
    result = compositor.assemble()
    #expect(result.values == ["大樹", "🆕", "蜜蜂"])
  }
  ...
  func test18_Composer_PerceptionDataTest_SaisoukiNoGaika() throws {
    ...
    let compositor = Megrez.Compositor(with: lm, separator: "-")
    let readingKeys = ["zai4", "chuang4", "shi4", "de5", "kai3", "ge1"]
    for key in readingKeys {
      #expect(compositor.insertKey(key))
    }
    compositor.assemble()
    let assembledPriorToOverride = compositor.assembledSentence
      .map(\.value)
      .joined(separator: " ")
    ...
    guard let keyForQueryingDataAt2 = compositor.assembledSentence
      ...
      Issue.record("Expected key for cursor \(cursorShi)")
      return
    }
    ...
    guard let keyForQueryingDataAt3 = compositor.assembledSentence
      ...
      Issue.record("Expected key for cursor \(cursorShiDe)")
      return
    }
    ...
    #expect(obsCaptured?.contextualizedGramKey == "(chuang4,創)&(shi4,世)&(shi4-de5,是的)")
    ...
    guard let afterHit = currentAssembly.findGram(at: cursorShiDe) else {
      Issue.record("Expected current gram at cursor \(cursorShiDe)")
      return
    }
    ...
    guard let prevHit = prevAssembly.findGram(at: innerIndex) else {
      Issue.record("Expected previous gram at cursor \(innerIndex)")
      return
    }
  ...
  func test19_Composer_PerceptionDataTest_BusinessEnglishSession() throws {
    ...
    guard let keyForQueryingDataAt5 = compositor.assembledSentence
      ...
      Issue.record("Expected key for cursor \(cursorHua)")
      return
    }
  ...
  func test20_Composer_PerceptionDataTest_DiJiaoSubmission() throws {
    ...
    compositor.assemble()
    let assembledFollowingFirstOverride = compositor.assembledSentence
      .map(\.value)
      .joined(separator: " ")
    #expect(["第 交", "第 教"].contains(assembledFollowingFirstOverride))
    ...
    guard let diJiaoCandidate = candidatesAtEnd.first(where: { $0.value == "遞交" }) else {
      Issue.record("遞交 should be available as a candidate ending at the current cursor.")
      return
    }
    ...
    guard let obsCaptured else {
      Issue.record("Perception intel should be captured when overriding with 遞交.")
      return
    }
    ...
    compositor.assemble()
    let assembledFollowingSecondOverride = compositor.assembledSentence
      .map(\.value)
      .joined(separator: " ")
    #expect("遞交" == assembledFollowingSecondOverride)

    let validationCompositor = Megrez.Compositor(with: lm)
    for key in readingKeys {
      #expect(validationCompositor.insertKey(key))
    }
    ...
    let assembledBySuggested = validationCompositor.assembledSentence
      .map(\.value)
      .joined(separator: " ")
    #expect("遞交" == assembledBySuggested)
  }
}
