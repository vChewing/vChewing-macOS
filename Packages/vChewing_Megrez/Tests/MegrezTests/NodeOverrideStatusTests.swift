...
@Suite("NodeOverrideStatus")
final class NodeOverrideStatusTests {
  /// 測試 FIUUID 的基本功能
  @Test
  func testFIUUID() throws {
    ...
    #expect(uuidString.allSatisfy { $0 == "-" || "0123456789ABCDEF".contains($0) })
  ...
  func testNodeOverrideStatus() throws {
    ...
    #expect(status.overridingScore == 100.0)
    #expect(status.currentOverrideType == .withSpecified)
    #expect(status.currentUnigramIndex == 2)
  ...
  func testNodeIdAndOverrideStatus() throws {
    ...
    #expect(modifiedStatus.overridingScore == baselineOverrideScore)
    #expect(modifiedStatus.currentOverrideType == .withSpecified)
    #expect(modifiedStatus.currentUnigramIndex == 0)
    ...
    #expect(node1.overridingScore == 300.0)
    #expect(node1.currentOverrideType == .withTopGramScore)
    #expect(node1.currentUnigramIndex == 0)
  }
  ...
  func testNodeOverrideStatusOverflowProtection() throws {
    ...
    #expect(node.currentUnigramIndex == 0)
  }
  ...
  func testCompositorNodeOverrideStatusMirror() throws {
    ...
    if let node = compositor.segments[1][2] {
      #expect(node.overridingScore == baselineOverrideScore)
      #expect(node.currentOverrideType == .withTopGramScore)
    }
  }
  ...
  func testLightweightStatusCopyVsFullCopy() throws {
    ...
    #expect(fullyCopiedCompositor.segments.count == compositor.segments.count)
