...
struct HybridPriorityQueue<T: Comparable> {
  ...
  mutating func enqueue(_ element: T) {
    if _fastPath(usingArray) {
      ...
      while left < right {
        ...
        // 減少分支：使用位元運算替代條件運算
        let comparison = isReversed ? sortedArray[mid] < element : sortedArray[mid] > element
        // 使用位元遮罩進行條件移動
        right = comparison ? mid : right
        left = comparison ? left : mid + 1
      }
  ...
  mutating func dequeue() -> T? {
    ...
    if _fastPath(usingArray) {
      return isReversed ? sortedArray.removeLast() : sortedArray.removeFirst()
    }
    return isReversed ? heap.remove() : heap.remove()
  }
