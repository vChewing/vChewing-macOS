...
extension SessionProtocol {
  ...
  public func commit(text: String, clearDisplayBeforeCommit: Bool) {
    ...
    var replaced = false
    if phE, let queried = inputHandler?.currentLM.lxQuerier.replacementValue(for: text) {
      replaced = true
      text = queried
    }
    var buffer = SessionHost.shared.kanjiConversionIfRequired(text)
    if phE, !replaced, let queried = inputHandler?.currentLM.lxQuerier.replacementValue(for: buffer) {
