...
extension InputHandlerProtocol {
  ...
  func handleCandidate(input: InputSignalProtocol, ignoringModifiers: Bool = false) -> Bool {
    ...
    if state.type == .ofInputting {
      ...
      // 狂拼模式：前方候選就地選字亦以 Shift+選字鍵觸發（逐字選字模式停用時）。
    ...
    if prefs.useSCPCTypingMode, state.type != .ofInputting {
      ...
      if let punctuationNamePrefix = generatePunctuationNamePrefix(withKeyCondition: input) {
        ...
        shouldAutoSelectCandidate = shouldAutoSelectCandidate
          || currentLM.lxQuerier.hasUnigrams(for: [customPunctuation])
          || currentLM.lxQuerier.hasUnigrams(for: [punctuation])
      }

      if !shouldAutoSelectCandidate, input.isUpperCaseASCIILetterKey {
        let letter = "_letter_\(inputText)"
        if currentLM.lxQuerier.hasUnigrams(for: [letter]) { shouldAutoSelectCandidate = true }
      }
