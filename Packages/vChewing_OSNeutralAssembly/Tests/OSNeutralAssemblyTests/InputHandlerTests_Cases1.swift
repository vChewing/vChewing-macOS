...
extension InputHandlerTests {
  ...
  func test_IH103D_ButKoBPMFVSMarkingStateDoesNotPollute() throws {
    ...
    // （tooltip 內容在 MainAssembly 層產生，OSNeutralAssembly 測試環境不連結 MainAssembly 故不檢查 tooltip 字串）
    #expect(
  ...
  /// OSNeutralAssembly 測試層級驗證 raw text 傳遞正確性；tooltip 內容驗證由 MainAssembly 層測試負責。
  @Test
  ...
  /// 打「ma」時，unigram「媽」由 OSNeutralAssembly 通道置頂（fetch 開）；「以固定順序陳列候選字」
