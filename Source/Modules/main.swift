...
guard let mainNibName = Bundle.main.infoDictionary?["NSMainNibFile"] as? String else {
  String.consoleLog("vChewingDebug: Fatal error: NSMainNibFile key not defined in Info.plist.")
  exit(-1)
}

let loaded = Bundle.main.loadNibNamed(mainNibName, owner: NSApp, topLevelObjects: nil)
if !loaded {
  String.consoleLog("vChewingDebug: Fatal error: Cannot load \(mainNibName).")
  exit(-1)
}
