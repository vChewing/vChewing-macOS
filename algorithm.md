...
鍵序列長度為 4 時，若在位置 0 有單字節點「A」、位置 2 有雙字節點「CD」，則可能最佳路徑為「A → B → CD」，總分為各節點分數加總。Homa 會透過 dp/parent 自動回溯得到該輸出序列。更多範例請洽 Homa 與 Typewriter 的單元測試。

---

## 語言模型匯流：LangModelAssembly

LangModelAssembly 對多個子語言模型進行匯整、去重、替換與增益調整，對外提供以「鍵序列」為鍵的 Homa Gram 陣列。
...
- 文件版本：1.2
- 最後更新：2026-04-27T01:57:00+08:00
- 適用版本：vChewing 4.4.0 及以上
