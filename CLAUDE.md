...
- **Scope**：主要工作區位於 `Packages/`。Linux 環境僅構建 `vChewing_OSNeutralAssembly` 與其依賴。macOS 構建使用 `Package.swift` + `Makefile` + `BundleApps` CommandPlugin。
...
- **FSM 流程**：維持 `InputSession → InputHandler (→ Homa) → IMEState` 流程；新增 API 時先更新協定（`SessionCoreProtocol` 提供共用的 `switchState()`/`resetInputHandler()` 預設實作；`InputHandlerProtocol` 處理輸入事件分診）。Sessions 體系已遷移至 OSNeutralAssembly 且 OS-independent——所有 OS-dependent 動作（LXMgr、IMEApp、Notifier、AppDelegate、NS*、IMKHelper 等）一律經 `SessionHost` 閉包注入（見 `SessionHostWiring.swift`），不要在 portable 會話程式碼內直接呼叫 Darwin 專屬 API。
