...
>
>**此 fork 新增之功能（个人修改版）：**
>- **智能中英文切换**：注音输入期间自动侦测英文词汇，可流畅切换至英文缓冲区输入。
>- **数字快打**（连按两次 `` ` `` 键触发）：快速插入中文数字、大写数字、日期、时间等格式。
>- **近音表选字**（↑ 键触发）：展开二维近音／同音候选表，方便事后修改罕用字。
>- **自动括号配对**：全形与半形括号输入时自动补完右括号。
>- **自动学习词语**：选字达到门槛次数后自动写入用户词典。
>- **二维分类符号表格**（单按 `` ` `` 键触发）：启用后可调用二维分类符号表格，提供常用、表情符号、标点、括号等多种分类，支援翻页浏览；Ctrl+`` ` `` 则开启旧式清单式符号表。
