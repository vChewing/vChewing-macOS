...
| REQ-004 | Tab 鍵提交英文緩衝後讓 Tab 穿透給應用程式 | P0 | 已實作（2026.04.04 更新） |
| REQ-005 | Backspace 逐字刪除英文緩衝，清空後返回中文模式 | P0 | 已實作 |
| REQ-006 | ~~輸入標點符號自動返回中文模式~~ → ASCII 可印列字元（含標點）加入英文緩衝 | P0 | 2026.04.04 修正 |
...
**臨時英文模式下的按鍵行為：**
- **字母、數字、ASCII 標點（0x21–0x7E）** → 加入英文緩衝（包含 `.`, `,`, `@`, `/` 等，可輸入 `cd ..`、`test@example.com` 等）
- **空白鍵** → 插入空格到英文緩衝
- **Tab 鍵** → 提交凍結段落＋英文緩衝，然後讓 Tab 穿透給應用程式（如移至下一欄位）
- **Enter 鍵** → 提交凍結段落＋英文緩衝，Enter 本身被消耗（不穿透給應用程式）
- **Backspace** → 逐字刪除英文緩衝，清空後返回中文模式
- **Esc** → 丟棄所有內容，返回空白狀態
...
2. 若處於臨時英文模式（isTempEnglishMode）：
   ├─ ASCII 可印列字元（0x21–0x7E，含字母、數字、標點）→ 加入 englishBuffer，顯示緩衝
   ├─ Space → 插入空格到 englishBuffer，顯示緩衝
   ├─ Tab → commit frozenSegments＋englishBuffer，讓 Tab 穿透給應用程式
   ├─ Enter → commit frozenSegments＋englishBuffer，Enter 被消耗（不穿透）
   └─ Backspace：
