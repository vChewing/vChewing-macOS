...
- **智慧中英文切換 — 凍結段落全面可編輯**：凍結段落（中英切換時保留在組字區的前置文字）現在支援完整游標操作：
  - **游標導航**：在組字區末端繼續按 ←/→，游標可進入凍結段落；從凍結段落末端按 → 自動退出，回到一般組字區。
  - **刪除字元**：游標位於凍結段落內時，Backspace 可逐字刪除凍結字元；刪除後若凍結段落已空，自動退出凍結游標模式。
  - **插入 ASCII 字元**：在凍結段落內可直接鍵入可列印 ASCII 字元（英數、標點）於游標位置插入。
  - **中文字重新選候選字詞**：游標在凍結段落內的中文字前方時，按 Space 或 ↓ 可開啟候選選字窗，對該字進行重新選字；選字完成後，凍結段落內容原地更新，游標停在替換字後方，其餘前後文完整保留。
  - **浮動組字窗（PCB）全文顯示**：候選選字窗開啟及導航過程中，浮動組字窗始終呈現完整句子脈絡（凍結前綴 + 候選字 + 凍結後綴），反白範圍精確對應當前候選字，不誤套前置文字。
...
- **智慧中英文切換 — 候選預覽丟失前後文**：修正凍結段落候選重選時，浮動組字窗（PCB）在使用者移動反白時只顯示單一候選字（如「計」），而非完整句子脈絡（如「你有收到我計出的Email嗎？」）的問題。根本原因：`previewCurrentCandidateAtCompositionBuffer()` 覆寫 `displayTextSegments` 時未考慮凍結前後綴；同時 `marker` 未加凍結前綴偏移，導致反白誤套前置文字。修正方式：在凍結候選模式下，`displayTextSegments`、`cursor`、`marker` 均嵌入 `frozenTextSplitAroundCandidatePosition()` 結果並加上正確偏移量。
